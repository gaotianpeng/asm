...
; 系统核心的头部，用于加载核心程序
        ...
        sys_routine_seg         dd  section.sys_routine.start    ; 系统公用例程段位置 0x04
        core_data_seg           dd  section.core_data.start      ; 核心数据段位置 0x08
        core_code_seg           dd  section.core_code.start      ; 核心代码段位置 0x0c

        core_entry              dd  start                        ; 核心代码段入口点 0x10
                                dw  core_code_seg_sel            ; 代码段选择子
...
put_string:                                 ; 显示0终止的字符串并移动光标
  ...
  .getc:
         mov cl, [ebx]
         or cl, cl
         jz .exit                           ; 前一个操作产生的结果为0,则跳转到指定的目标地址
         call put_char
         inc ebx
         jmp .getc
...
put_char:                                   ; 在当前光标处显示一个字符,并推进
    ...
    .roll_screen:
        ...
        mov ecx, 1920
        rep movsd                           ; 源 ds:esi 目的 es:edi
        mov bx, 3840                        ; 清除屏幕最底一行
        mov ecx, 80                         ; 32位程序应该使用 ecx
    .cls:
...
load_relocate_program:                          ; 加载并重定位用户程序
        ...
        mov ebx, core_buf
        call sys_routine_seg_sel:read_hard_disk
...
start:
        ...
        ; 显示处理器品牌信息
        mov eax, 0x80000002             ; eax 用于指定要返回什么样的信息
        cpuid                           ; cpuid 用于返回处理器的标识和特性信
                                        ; cpuid 执行后，处理器将返回的信息放在eax,ebx,ecx,edx中
